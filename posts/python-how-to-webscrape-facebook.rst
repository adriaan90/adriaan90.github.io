...
.. post:: Apr 4, 2020
   :exclude:
   :tags: tutorial, python, webscrape, requests, beautifulsoup
   :author: adriaan
...
and then randomly select a winner from that group of people. 
...
It will take quite a lot of time to determine the winner then...
...
We are interested in the names of people who follow the business's page and who liked and shared the post in question. 
All of this information is available on Facebook. 
In the end we will have three lists:
...
First thing we need to do is to get the HTML data of the Facebook post we are interested in, into our Python script. 
We can do this using the Python library, ``requests``. 

Web scraping Facebook is a bit different, as the content is behind a login page.
We will need to first have a look at the HTML code of the login page. 
For this project, we are going to log into ``https://mbasic.facebook.com``:
...
Also, the list is not shown when you access it through the website address ``https://mbasic.facebook.com`` and ``https://m.facebook.com`` and we will thus need another approach to get the HTML data through the use of the Selenium library:
...
The ``sleep()`` command gives the page time to load before entering the login details. 
Next we navigate to the page settings listing all the names:
